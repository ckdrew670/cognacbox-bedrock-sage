@php
		...
		$formatted_authors = count(get_field('paper_authors')) > 1 ? 'Authors: ' . $authors_list : 'Author: ' . $authors_list;
		$abstract = get_field('paper_abstract');
		$publication_date = new DateTime(strtok(get_field('paper_date'), " "));
		$formattedPublicationDate = $publication_date->format('M d, Y');
...
<main id="app">
	...
	@include('patterns.hero.hero', [
		...
		'copy' => $hero_copy,
		'formatted_post_date' => $formattedPublicationDate,
